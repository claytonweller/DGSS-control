...
export function Trolly({ moduleState, sendAction }) {
  ...
  const examples = () => {
    return (
      <div>
        <div>
          <button onClick={() => questionClick('5 people', '1 person')}>1 Basic</button>
          <button onClick={() => questionClick('5 people', 'Snipe Trolly driver')}>2 Sniper</button>
          <button onClick={() => questionClick('5 people', 'Shove person in front of Trolly')}>3 Shove</button>
          <button onClick={() => questionClick('1 baby', '1 adult', 1750)}>4 Timer</button>
        </div>
        <div>
